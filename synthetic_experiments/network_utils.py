...
def Row(matrix):
    """
    Normalizes the rows of a matrix to make it row-stochastic.
    Each row in the output matrix sums to 1.

    Args:
        matrix (np.ndarray): The input matrix. Shape: (n, n).

    Returns:
        np.ndarray: The row-stochastic matrix. Shape: (n, n).
    """
...
def Col(matrix):
    """
    Normalizes the columns of a matrix to make it column-stochastic.
    Each column in the output matrix sums to 1.

    Args:
        matrix (np.ndarray): The input matrix. Shape: (n, n).

    Returns:
        np.ndarray: The column-stochastic matrix. Shape: (n, n).
    """
...
def generate_exp_matrices(n, seed=42):
    """
    Generates row-stochastic (A) and column-stochastic (B) matrices for an exponential graph topology.
    In an exponential graph, nodes are connected to their neighbors at distances that are powers of 2.
    ...
    Returns:
        tuple: A tuple containing:
            - A (np.ndarray): The row-stochastic matrix. Shape: (n, n).
            - B (np.ndarray): The column-stochastic matrix. Shape: (n, n).
    """
    def generate_exponential_weight_matrix(n):
        """Helper to create the base weight matrix for an exponential graph."""
        denominator = np.abs(np.log2(n)) + 1
        i_indices, j_indices = np.indices((n, n))
        mod_vals = (j_indices - i_indices) % n
        # An edge exists if the difference between indices is a power of two
    ...
    np.random.seed(seed)
    # Assign random integer weights where edges exist
    ...
    A = Row(M)  # Make it row-stochastic
    B = Col(M)  # Make it column-stochastic
    return A, B

def generate_grid_matrices(n, seed=42):
    """
    Generates stochastic matrices for a grid graph topology.
    The number of nodes `n` must be a perfect square.

    Args:
        n (int): The number of nodes (must be a perfect square).
        seed (int): Random seed for reproducibility.

    Returns:
        tuple or None: A tuple (A, B) of stochastic matrices, or None if n is not a perfect square.
            - A (np.ndarray): The row-stochastic matrix. Shape: (n, n).
            - B (np.ndarray): The column-stochastic matrix. Shape: (n, n).
    """
...
def generate_ring_matrices(n, seed=42):
    """
    Generates stochastic matrices for a ring graph topology with some extra connections.
    ...
    Returns:
        tuple or None: A tuple (A, B) of stochastic matrices, or None if n < 2.
            - A (np.ndarray): The row-stochastic matrix. Shape: (n, n).
            - B (np.ndarray): The column-stochastic matrix. Shape: (n, n).
    """
    if n < 2:
    ...
    for i in range(n):
        N[i, i] = 1  # Self-loop
        N[i, (i + 1) % n] = 1  # Connect to the next neighbor in the ring
    
    # Add some cross-connections for faster mixing
    idx_n_4 = int(n / 4)
    idx_n_2 = int(n / 2)
    idx_3n_4 = int(3 * n / 4)
    if 0 <= idx_n_4 < n:
...
def generate_random_graph_matrices(n: int, seed: int = 42):
    """
    Generates stochastic matrices for a random graph (Erdos-Renyi style).
    ...
    Returns:
        tuple: A tuple (A, B) of stochastic matrices.
            - A (np.ndarray): The row-stochastic matrix. Shape: (n, n).
            - B (np.ndarray): The column-stochastic matrix. Shape: (n, n).
    """
    np.random.seed(seed)
    ...
    # For each pair of nodes, create an edge with a certain probability
    for i in range(n):
        for j in range(i + 1, n):  # Iterate over upper triangle to avoid duplicates
            if np.random.rand() < prob_edge_exists:
                N[i, j] = 1
                N[j, i] = 1  # Symmetric connection
...
def generate_stochastic_geometric_matrices(n, seed, threshold=5):
    """
    Generates stochastic matrices for a random geometric graph.
    Nodes are placed randomly in a 2D space, and edges connect nodes within a certain distance.

    Args:
        n (int): The number of nodes (>= 6).
        seed (int): Random seed for reproducibility.
        threshold (float): The distance threshold for creating an edge.
    ...
    np.random.seed(seed)
    positions = np.random.rand(n, 2) * 10  # Random positions in a 10x10 square
    W = np.zeros((n, n))
    np.fill_diagonal(W, 1)  # Self-loops
    
    # Connect nodes if their Euclidean distance is within the threshold
    for i in range(n):
...
def generate_nearest_neighbor_matrices(n: int, k: int = 3, seed: int = 42):
    """
    Generates stochastic matrices for a k-nearest neighbor graph.
    Each node is connected to its `k` closest neighbors in a 2D space.
    ...
    if n > 1 and k > 0:
        ...
        for i in range(n):
            ...
            # Connect to the k nearest neighbors (excluding self, which is at index 0)
            for neighbor_idx in sorted_neighbor_indices[1 : k + 1]:
                W[i, neighbor_idx] = 1
                W[neighbor_idx, i] = 1  # Symmetric connection
