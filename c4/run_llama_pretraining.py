...
def parse_args(args):
    ...
    supported_optimizers = ['adamw','galore_adamw','golore_adamw', 'adam_meso']
...
def main(args):
    ...
    if args.optimizer.lower() == "adam_meso":
        from optimizer.adam_meso import MixedLoRAModel, AdamW_meso
        model = MixedLoRAModel(
            model,
            r_1=args.compress_rank,
            r_2=args.compress_rank,
            r_3=args.compress_rank,
    ...
    if args.optimizer.lower() == "adamw":
    ...
    elif args.optimizer.lower() == "adam_meso":
        from optimizer.adam_meso import MixedLoRAModel, AdamW_meso
        qk_params = []
        vo_params = []  
        mlpup_params = []
        ...
        for n, p in model.named_parameters():
            ...
            if "q_proj" in n or "k_proj" in n:
                print(n, "1")
                qk_params.append(p)
            elif "v_proj" in n or "o_proj" in n:
                print(n, "2")
                vo_params.append(p)
            elif "mlp" in n and ("up" or "gate") in n:
                print(n, "3")
                mlpup_params.append(p)
            elif "mlp" in n and "down" in n:
                print(n, "4")
                mlpdown_params.append(p)
            elif "embed" in n:
                embed_params.append(p)
            else:
                ln_params.append(p)

        optimizer = AdamW_meso(
            [
                {"params": embed_params, "lr": args.lr},
                {"params": ln_params, "lr": args.lr},
                {"params": qk_params + vo_params, "lr": args.lr, "scaling_factor": 0.05},
                {"params": mlpup_params + mlpdown_params, "lr": args.lr, "scaling_factor": 0.05}, 
            ],
        ...
        from optimizer import GoLoreAdamW
        from glue.utils import get_galore_param_groups
        galore_param = get_galore_param_groups(model, weight_decay=args.weight_decay,rank=args.rank,
                                                    update_proj_gap=args.update_proj_gap,scale=args.scale,proj_type=args.proj_type)
        optimizer = GoLoreAdamW(galore_param, lr=args.lr, betas=(args.beta1,args.beta2), eps=args.eps, weight_decay=args.weight_decay)
    else:
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if update_step % args.update_proj_gap == 1 and args.optimizer == 'adam_meso':
            _lora_reset_time = time.time()
            logger.info(f"{local_step=}, {args.update_proj_gap=}, thresh: {local_step // args.gradient_accumulation}")
